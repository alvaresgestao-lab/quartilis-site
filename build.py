#!/usr/bin/env python3
"""Gera o site estático da Quartilis a partir de content/*.json.
Uso: python3 build.py
"""
import json, re, os, urllib.parse, html

ROOT = os.path.dirname(os.path.abspath(__file__))
def J(name): return json.load(open(os.path.join(ROOT, 'content', name), encoding='utf-8'))

MUSICOS = J('musicos.json')
GALERIA = J('galeria.json')
TITLES = J('video-titles.json')
SOCIALS = J('socials.json')

WHATS = '5541991271129'
WHATS_FMT = '(41) 99127-1129'
EMAIL = 'contato@quartilis.com.br'

def esc(s): return html.escape(s, quote=False)
def imgname(u): return urllib.parse.unquote(u.split('?')[0].rsplit('/', 1)[-1])

NAV = [('', 'Home'), ('musicos/', 'Músicos'), ('servicos/', 'Serviços'),
       ('fotos-e-videos/', 'Fotos e Vídeos'), ('repertorio/', 'Repertório'),
       ('https://serenata.quartilis.com.br', 'Serenatas'),
       ('https://blog.quartilis.com.br', 'Blog'), ('contato/', 'Contato')]

def header(prefix, active):
    items = ''
    for href, label in NAV:
        url = href if href.startswith('http') else prefix + href
        cls = ' class="ativo"' if label == active else ''
        ext = ' target="_blank" rel="noopener"' if href.startswith('http') else ''
        items += f'<li><a href="{url or prefix or "./"}"{cls}{ext}>{label}</a></li>'
    return f'''<header class="topo">
  <div class="topo-inner">
    <a class="marca" href="{prefix or './'}"><img class="marca-logo" src="{prefix}assets/logo-quartilis.png?v=1" alt=""><span class="marca-textos">QUARTILIS<span>música para eventos</span></span></a>
    <input type="checkbox" id="menu-chk" aria-label="Abrir menu">
    <label for="menu-chk" class="menu-btn"><span></span><span></span><span></span></label>
    <nav><ul>{items}</ul></nav>
  </div>
</header>'''

def footer(prefix):
    soc = ''
    icons = {'instagram': 'M12 2.2c3.2 0 3.6 0 4.9.1 3.3.1 4.8 1.7 4.9 4.9.1 1.3.1 1.6.1 4.8s0 3.6-.1 4.8c-.1 3.2-1.7 4.8-4.9 4.9-1.3.1-1.6.1-4.9.1-3.2 0-3.6 0-4.8-.1-3.3-.1-4.8-1.7-4.9-4.9-.1-1.3-.1-1.6-.1-4.8s0-3.6.1-4.8C2.4 4 4 2.4 7.2 2.3 8.4 2.2 8.8 2.2 12 2.2zm0 3.6a6.2 6.2 0 1 0 0 12.4 6.2 6.2 0 0 0 0-12.4zm0 10.2a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.4-10.5a1.4 1.4 0 1 1-2.9 0 1.4 1.4 0 0 1 2.9 0z',
             'facebook': 'M13.5 21v-8.2h2.8l.4-3.2h-3.2V7.6c0-.9.3-1.6 1.6-1.6h1.7V3.1c-.3 0-1.3-.1-2.5-.1-2.5 0-4.2 1.5-4.2 4.3v2.4H7.3v3.2h2.8V21h3.4z',
             'youtube': 'M21.6 7.2a2.5 2.5 0 0 0-1.8-1.8C18.2 5 12 5 12 5s-6.2 0-7.8.4A2.5 2.5 0 0 0 2.4 7.2 26 26 0 0 0 2 12c0 1.6.1 3.2.4 4.8a2.5 2.5 0 0 0 1.8 1.8c1.6.4 7.8.4 7.8.4s6.2 0 7.8-.4a2.5 2.5 0 0 0 1.8-1.8c.3-1.6.4-3.2.4-4.8s-.1-3.2-.4-4.8zM10 15.2V8.8l5.2 3.2L10 15.2z'}
    for k, url in SOCIALS.items():
        if k in icons:
            soc += f'<a href="{url}" target="_blank" rel="noopener" aria-label="{k}"><svg viewBox="0 0 24 24" fill="currentColor"><path d="{icons[k]}"/></svg></a>'
    return f'''<footer class="rodape">
  <div class="rodape-inner">
    <div>
      <p class="marca-rodape">QUARTILIS</p>
      <p>Música ao vivo para casamentos e eventos, em Curitiba e região, desde 2002.</p>
    </div>
    <div>
      <p><a href="https://wa.me/{WHATS}" target="_blank" rel="noopener">WhatsApp {WHATS_FMT}</a></p>
      <p><a href="mailto:{EMAIL}">{EMAIL}</a></p>
      <div class="social">{soc}</div>
    </div>
  </div>
  <p class="assinatura">© Quartilis · música para eventos · Site por <a href="https://pedroalvares.com/" target="_blank" rel="noopener">Pedro Álvares</a></p>
</footer>
<div class="whats-widget" id="wwidget">
  <div class="whats-painel" id="wpainel">
    <h3>Fale com o Quartilis</h3>
    <p>Conte pra gente o motivo do contato e continue a conversa no WhatsApp.</p>
    <label for="wnome">Seu nome</label>
    <input id="wnome" type="text" placeholder="Como podemos te chamar?">
    <label for="wmotivo">Motivo do contato</label>
    <select id="wmotivo">
      <option>Casamento</option>
      <option>Serenata ou homenagem</option>
      <option>Aniversário ou data especial</option>
      <option>Evento corporativo</option>
      <option>Missa ou celebração religiosa</option>
      <option>Outro assunto</option>
    </select>
    <button class="whats-enviar" onclick="wEnviar()">Iniciar conversa</button>
  </div>
  <button class="whats-btn" onclick="wAbrir()" aria-label="Falar no WhatsApp">
  <svg viewBox="0 0 32 32" fill="currentColor"><path d="M16 3C9 3 3.3 8.7 3.3 15.7c0 2.4.7 4.7 1.9 6.7L3 29l6.8-2.1c1.9 1 4 1.6 6.2 1.6 7 0 12.7-5.7 12.7-12.7S23 3 16 3zm0 23.1c-2 0-3.9-.6-5.6-1.6l-.4-.2-4 1.2 1.2-3.9-.3-.4a10.2 10.2 0 0 1-1.6-5.5C5.3 10.1 10.1 5.3 16 5.3s10.7 4.8 10.7 10.4S21.9 26.1 16 26.1zm5.9-7.7c-.3-.2-1.9-1-2.2-1.1-.3-.1-.5-.2-.7.2-.2.3-.8 1.1-1 1.3-.2.2-.4.2-.7.1-.3-.2-1.4-.5-2.6-1.6-1-.9-1.6-1.9-1.8-2.2-.2-.3 0-.5.1-.7l.5-.6c.2-.2.2-.3.3-.6.1-.2 0-.4 0-.6l-1-2.4c-.3-.6-.5-.5-.7-.5h-.6c-.2 0-.6.1-.9.4-.3.3-1.1 1.1-1.1 2.7s1.2 3.1 1.3 3.4c.2.2 2.3 3.5 5.5 4.9.8.3 1.4.5 1.8.7.8.2 1.5.2 2 .1.6-.1 1.9-.8 2.1-1.5.3-.7.3-1.4.2-1.5l-.5-.3z"/></svg></button>
</div>
<script>
function wAbrir(){{document.getElementById('wpainel').classList.toggle('aberto')}}
function wEnviar(){{var n=document.getElementById('wnome').value.trim();var m=document.getElementById('wmotivo').value;var W='https://script.google.com/macros/s/AKfycby3-pZROVh0aF1XMCpRcFV-PZxSwYpY_v84X8nL4gQ5qwNLAq9rVPIwMtzYZIlh9Csm/exec';try{{new Image().src=W+'?nome='+encodeURIComponent(n)+'&motivo='+encodeURIComponent(m)+'&site='+encodeURIComponent(location.hostname)+'&pagina='+encodeURIComponent(location.pathname)+'&ref='+encodeURIComponent(document.referrer||'direto');}}catch(err){{}}var t='Olá! '+(n?('Meu nome é '+n+', vim'):'Vim')+' através do site do Quartilis e gostaria de falar sobre: '+m+'.';window.open('https://wa.me/5541991271129?text='+encodeURIComponent(t),'_blank')}}
document.addEventListener('click',function(e){{var a=e.target.closest('a[href^="https://wa.me"]');if(!a)return;e.preventDefault();document.getElementById('wpainel').classList.add('aberto');var c=document.getElementById('wnome');if(c)c.focus()}});
</script>'''

def page(prefix, active, title, desc, body, extra_head=''):
    return f'''<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{title}</title>
<meta name="description" content="{desc}">
<link rel="canonical" href="__CANONICAL__">
<link rel="icon" type="image/png" href="{prefix}assets/favicon.png?v=1">
<link rel="apple-touch-icon" href="{prefix}assets/apple-touch-icon.png">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@500;600&family=Jost:wght@300;400;500&display=swap" rel="stylesheet">
<link rel="stylesheet" href="{prefix}assets/site.css?v=4">
{extra_head}</head>
<body>
{header(prefix, active)}
{body}
{footer(prefix)}
</body>
</html>'''

def write(path, content):
    full = os.path.join(ROOT, path)
    os.makedirs(os.path.dirname(full), exist_ok=True)
    canonical = 'https://quartilis.com.br/' + path[:-len('index.html')] if path.endswith('index.html') else 'https://quartilis.com.br/' + path
    content = content.replace('__CANONICAL__', canonical)
    open(full, 'w', encoding='utf-8').write(content)
    print(f'  {path} ({len(content)//1024} KB)')

I = 'assets/img/'

# ============ HOME ============
body = f'''
<section class="hero-nova">
  <div class="fundo-f" aria-hidden="true"><span>ƒ</span><span class="espelhado">ƒ</span></div>
  <div class="hero-grade">
    <div class="hero-esq">
      <p class="kicker">Música para eventos · Curitiba · desde 2002</p>
      <h1>Música ao vivo que emociona</h1>
      <p class="hero-sub">Do violino solo à orquestra de cordas e clarins, formações personalizadas para casamentos e eventos.</p>
      <div class="hero-botoes">
        <a class="btn" href="https://wa.me/{WHATS}?text=Olá!%20Gostaria%20de%20um%20orçamento." target="_blank" rel="noopener">Pedir orçamento</a>
        <a class="btn btn-borda" href="repertorio/">Ver repertório</a>
      </div>
    </div>
    <figure class="hero-dir foto-chanfro"><img src="{I}KG9A0098.jpg" alt="Concerto à luz de velas do Quartilis" fetchpriority="high"></figure>
  </div>
</section>

<section class="secao">
  <div class="secao-inner duas-colunas">
    <div>
      <h2>O Quartilis</h2>
      <p>Quartilis é um grupo especializado em música para eventos. Surgiu em 2002 em formato de Quarteto de Cordas e hoje é formado por 13 instrumentistas, oferecendo várias formações musicais que se adaptam a cada momento e estilo de evento.</p>
      <p>O grupo é reconhecido pela alta qualidade, afinidade entre os músicos e a execução impecável do repertório, do erudito ao popular.</p>
      <a class="link-seta" href="musicos/">Conheça os músicos</a>
    </div>
    <figure class="foto-chanfro"><img src="{I}quarteto-cordas-harpa-jardim-casamento-curitiba.jpg" alt="Quarteto de cordas e harpa do Quartilis em casamento ao ar livre em Curitiba" loading="lazy"></figure>
  </div>
</section>

<section class="secao secao-creme">
  <div class="secao-inner">
    <h2>O Quartilis em ação</h2>
    <p class="sub">Assista ao grupo ao vivo e conheça um repertório que vai do erudito aos clássicos do rock.</p>
    <div class="video-wrap" data-video="OrDxE1oDg54">
      <img src="https://i.ytimg.com/vi/OrDxE1oDg54/hqdefault.jpg" alt="Vídeo da semana do Quartilis" loading="lazy">
      <button class="play" aria-label="Assistir vídeo"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></button>
    </div>
    <p class="centro"><a class="btn" href="repertorio/">Conheça o nosso repertório</a></p>
  </div>
</section>

<section class="secao">
  <div class="secao-inner">
    <h2>Serviços</h2>
    <div class="cards-servicos">
      <div class="card-servico"><h3>Música ao vivo para cerimônia</h3><p class="tag">Casamento, bodas e missa</p><p>Formações que variam de instrumentos solo a orquestra de cordas e clarins, para emocionar e tornar o seu evento único.</p></div>
      <div class="card-servico"><h3>Música ao vivo para recepção</h3><p class="tag">Coquetel, almoço e jantar</p><p>Música instrumental requintada, com diversas opções de estilos e formações para deixar o ambiente sofisticado.</p></div>
      <div class="card-servico"><h3>Concerto didático</h3><p class="tag">Eventos e congressos</p><p>Para abertura de eventos, congressos ou palestras, com repertório variado para agradar a todos os públicos.</p></div>
      <div class="card-servico"><h3>Serenatas e homenagens</h3><p class="tag">Presenteie quem você ama</p><p>Homenagens musicais em diversas formações, para surpreender e emocionar em qualquer ocasião.</p></div>
    </div>
    <p class="centro"><a class="link-seta" href="servicos/">Todos os serviços</a></p>
  </div>
</section>

<section class="cta-final">
  <h2>Vamos montar a trilha sonora do seu evento?</h2>
  <a class="btn" href="https://wa.me/{WHATS}?text=Olá!%20Gostaria%20de%20um%20orçamento." target="_blank" rel="noopener">Falar no WhatsApp</a>
</section>
<script>
document.querySelectorAll('.video-wrap').forEach(function(w){{
  w.addEventListener('click', function(){{
    var id = w.dataset.video;
    w.outerHTML = '<div class="video-frame"><iframe src="https://www.youtube.com/embed/'+id+'?autoplay=1" title="Vídeo do Quartilis" allow="autoplay; encrypted-media" allowfullscreen></iframe></div>';
  }});
}});
</script>'''
write('index.html', page('', 'Home', 'Quartilis | Música ao vivo para casamentos e eventos em Curitiba',
      'Grupo especializado em música ao vivo para casamentos e eventos desde 2002. Formações do violino solo à orquestra, em Curitiba e região.', body))

# ============ MÚSICOS ============
cards = ''
for m in MUSICOS:
    nome = m['paras'][0]
    resto = m['paras'][1] if len(m['paras']) > 1 else ''
    linhas = resto.split('\n')
    instrumento = linhas[0].strip()
    bio = [' '.join(l.strip() for l in linhas[1:] if l.strip())] + [p for p in m['paras'][2:]]
    bio_html = ''.join(f'<p>{esc(b)}</p>' for b in bio if b)
    foto = imgname(m['img'])
    cards += f'''<article class="card-musico">
  <img src="../{I}{esc(foto)}" alt="{esc(nome)}" loading="lazy">
  <h3>{esc(nome)}</h3>
  <p class="instrumento">{esc(instrumento)}</p>
  <details><summary>Ler biografia</summary>{bio_html}</details>
</article>'''
body = f'''
<section class="secao secao-titulo"><div class="secao-inner"><h1>Músicos</h1>
<p class="sub">A equipe que dá vida ao Quartilis: instrumentistas com formação erudita e longa estrada em casamentos e eventos.</p></div></section>
<section class="secao"><div class="secao-inner"><div class="grade-musicos">{cards}</div></div></section>'''
write('musicos/index.html', page('../', 'Músicos', 'Músicos | Quartilis',
      'Conheça os instrumentistas do Quartilis: cordas, piano, harpa, sax e mais, com formação erudita e experiência em eventos.', body))

# ============ SERVIÇOS ============
servicos = [
    ('Música ao vivo para cerimônia', 'Casamento, bodas e missa',
     'Com formações que variam desde instrumentos solo (piano, violino, violão) até orquestra de cordas e clarins, com a participação de cantores, para emocionar e tornar o seu evento único e especial.',
     'clarins-flamula-quartilis-casamento-curitiba.jpg', 'Clarins do Quartilis na entrada da noiva em casamento em Curitiba'),
    ('Música ao vivo para recepção', 'Coquetel, almoço e jantar',
     'Música instrumental requintada, com diversas opções de estilos musicais e formações, incluindo grupos acústicos, para recepcionar os convidados e deixar o ambiente do seu evento agradável e sofisticado.',
     'sax-piano-recepcao-casamento-curitiba.jpg', 'Saxofone e piano na recepção de casamento em Curitiba'),
    ('Concerto didático', 'Abertura de eventos, congressos e palestras',
     'Repertório variado para agradar a todos os públicos e enriquecer o seu evento.',
     'trio-violino-violoncelo-harpa-casamento-curitiba.jpg', 'Trio de violino, violoncelo e harpa do Quartilis em Curitiba'),
    ('Serenatas e homenagens', 'Um presente inesquecível',
     'Homenagens musicais em diversas opções de formações, para presentear e homenagear quem você ama em aniversários, pedidos de casamento e datas especiais. <a class="link-seta" href="https://serenata.quartilis.com.br" target="_blank" rel="noopener">Conheça o site de Serenatas</a>',
     'violinista-casamento-curitiba.jpg', 'Violinista do Quartilis em serenata em Curitiba'),
]
blocos = ''.join(f'''<div class="card-servico grande"><figure class="card-foto"><img src="../{I}{img}" alt="{alt}" loading="lazy"></figure><h2>{t}</h2><p class="tag">{tag}</p><p>{d}</p></div>''' for t, tag, d, img, alt in servicos)
body = f'''
<section class="secao secao-titulo"><div class="secao-inner"><h1>Serviços</h1>
<p class="sub">O Quartilis oferece formações musicais personalizadas para o seu evento. Agende um atendimento e receba uma consultoria especializada para a escolha da formação e do repertório.</p></div></section>
<section class="secao"><div class="secao-inner"><div class="cards-servicos">{blocos}</div>
<p class="centro"><a class="btn" href="https://wa.me/{WHATS}?text=Olá!%20Gostaria%20de%20agendar%20uma%20consultoria." target="_blank" rel="noopener">Agendar consultoria</a></p></div></section>'''
write('servicos/index.html', page('../', 'Serviços', 'Serviços | Quartilis',
      'Música ao vivo para cerimônia e recepção, concertos didáticos, serenatas e homenagens. Formações personalizadas para o seu evento.', body))

# ============ FOTOS E VÍDEOS ============
fotos_html = ''
for f in GALERIA['fotos']:
    nome = imgname(f['full'])
    alt = f.get('alt') or nome.rsplit('.', 1)[0].replace('-', ' ').replace('_', ' ')
    fotos_html += f'<a class="foto" href="../{I}{esc(nome)}" target="_blank"><img src="../{I}{esc(nome)}" alt="{esc(alt)}" loading="lazy"></a>'
videos_html = ''
for v in GALERIA['videos']:
    vid = v['embed'].rsplit('/', 1)[-1]
    titulo = TITLES.get(vid) or 'Vídeo do Quartilis'
    videos_html += f'''<a class="card-video" href="https://www.youtube.com/watch?v={vid}" target="_blank" rel="noopener">
  <span class="thumb"><img src="https://i.ytimg.com/vi/{vid}/hqdefault.jpg" alt="{esc(titulo)}" loading="lazy"><span class="play-mini"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></span></span>
  <span class="titulo-video">{esc(titulo)}</span></a>'''
body = f'''
<section class="secao secao-titulo"><div class="secao-inner"><h1>Fotos e Vídeos</h1>
<p class="sub">Momentos reais de casamentos, concertos e eventos com o Quartilis.</p></div></section>
<section class="secao"><div class="secao-inner"><h2>Fotos</h2><div class="grade-fotos">{fotos_html}</div></div></section>
<section class="secao secao-creme"><div class="secao-inner"><h2>Vídeos</h2><div class="grade-videos">{videos_html}</div>
<p class="centro"><a class="link-seta" href="{SOCIALS.get('youtube','#')}" target="_blank" rel="noopener">Ver mais no canal do YouTube</a></p></div></section>'''
write('fotos-e-videos/index.html', page('../', 'Fotos e Vídeos', 'Fotos e Vídeos | Quartilis',
      'Galeria de fotos e vídeos do Quartilis em casamentos, concertos e eventos em Curitiba.', body))

# ============ CONTATO ============
body = f'''
<section class="secao secao-titulo"><div class="secao-inner"><h1>Contato</h1>
<p class="sub">Fale com a gente para montar a formação e o repertório ideais para o seu evento.</p></div></section>
<section class="secao"><div class="secao-inner contato-grade">
  <a class="cartao-contato" href="https://wa.me/{WHATS}?text=Olá!%20Gostaria%20de%20um%20orçamento." target="_blank" rel="noopener">
    <h3>WhatsApp</h3><p>{WHATS_FMT}</p><p class="tag">Resposta mais rápida</p></a>
  <a class="cartao-contato" href="mailto:{EMAIL}"><h3>E-mail</h3><p>{EMAIL}</p><p class="tag">Orçamentos e propostas</p></a>
  <div class="cartao-contato"><h3>Consultoria para noivos</h3><p>Atendimento personalizado para escolher a formação musical e o repertório ideais para a sua cerimônia.</p><p class="tag">Agende pelo WhatsApp</p></div>
</div></section>'''
write('contato/index.html', page('../', 'Contato', 'Contato | Quartilis',
      'Entre em contato com o Quartilis pelo WhatsApp (41) 99127-1129 ou contato@quartilis.com.br.', body))

# ============ REPERTÓRIO (integra nav/rodapé na página existente) ============
rep_path = os.path.join(ROOT, 'repertorio', 'index.html')
rep = open(rep_path, encoding='utf-8').read()
rep = re.sub(r'<header class="topo">.*?</header>\n?', '', rep, flags=re.S)
rep = re.sub(r'<footer class="rodape">.*?</footer>\n?', '', rep, flags=re.S)
rep = re.sub(r'<a class="whats-flutuante".*?</a>\n?', '', rep, flags=re.S)
# limpa TODAS as injeções acumuladas de builds anteriores (idempotente) antes de reinjetar uma única vez
rep = re.sub(r'<link rel="icon" href="data:image/svg\+xml,.*?</svg>">\n?', '', rep, flags=re.S)
rep = re.sub(r'<link rel="icon" type="image/png" href="\.\./assets/favicon\.png\?v=\d+">\n?', '', rep)
rep = re.sub(r'<link rel="apple-touch-icon" href="\.\./assets/apple-touch-icon\.png">\n?', '', rep)
rep = re.sub(r'<link rel="stylesheet" href="\.\./assets/site\.css(?:\?v=\d+)?">\n?', '', rep)
rep = re.sub(r'<link rel="canonical"[^>]*>\n?', '', rep)
rep = rep.replace('Repertório | Quartilis (protótipo)', 'Repertório | Quartilis')
rep = rep.replace('</head>', '<link rel="canonical" href="https://quartilis.com.br/repertorio/">\n<link rel="icon" type="image/png" href="../assets/favicon.png?v=1">\n<link rel="apple-touch-icon" href="../assets/apple-touch-icon.png">\n<link rel="stylesheet" href="../assets/site.css?v=4">\n</head>', 1)
rep = rep.replace('<body>', '<body>\n' + header('../', 'Repertório'), 1)
rep = rep.replace('</body>', footer('../') + '\n</body>', 1)
open(rep_path, 'w', encoding='utf-8').write(rep)
print('  repertorio/index.html (nav + rodapé integrados)')

print('OK')
